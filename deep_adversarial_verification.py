...
# 3. Real-Time 2FA TOTP Subscription Delivery
print("\n--- 3. Testing Real-Time 2FA TOTP Subscription Access ---", flush=True)
curr_totp = get_current_totp()
...
# Test TOTP directly in path with Base64 format (/sub/<6-digit-TOTP>/b64)
resp_totp_b64 = urllib.request.urlopen(f"http://{SERVER_IP}:{PORT}/sub/{curr_totp}/b64", timeout=5)
totp_b64_links = base64.b64decode(resp_totp_b64.read().decode()).decode().strip().split("\n")
check(len(totp_b64_links) == 7,
      f"Direct path TOTP Base64 subscription (/sub/{curr_totp}/b64) delivered 7 protocol links")

# Test TOTP via query param (/sub/totp?code=XXXXXX)
resp_totp_query = urllib.request.urlopen(f"http://{SERVER_IP}:{PORT}/sub/totp?code={curr_totp}", timeout=5)
check(resp_totp_query.status == 200,
      f"Query param TOTP subscription (/sub/totp?code={curr_totp}) delivered successfully")
...
login_totp_data = urllib.parse.urlencode({'auth_credential': curr_totp}).encode()
...
check(resp_totp.geturl() == f"http://{SERVER_IP}:{PORT}/portal" and "sf_session" in [c.name for c in cj_totp],
      f"Portal login with live 6-digit TOTP code ({curr_totp}) succeeded")
...
last_step_file = os.path.join(SCRATCH, ".last_ssh_step")
last_step = 0
...
env = os.environ.copy()
env["SSH_ASKPASS"] = os.path.join(SCRATCH, "askpass.exe")
