...
def get_current_totp():
    ...
    msg = struct.pack('>Q', int(time.time() + 2.3) // 30)
...
def wait_for_fresh_totp():
    ...
    time.sleep(wait)
    LAST_TEST_STEP = int(time.time() + 2.3) // 30
...
default_opener = urllib.request.build_opener(urllib.request.ProxyHandler({}), https_handler)
urllib.request.install_opener(default_opener)

def fetch_url(url, timeout=15):
    req = urllib.request.Request(url)
    resp = default_opener.open(req, timeout=timeout)
    data = resp.read()
    resp.close()
    return resp, data
...
print("\n--- 1. Testing Offline Embedded Asset Engine (over HTTPS) ---", flush=True)
resp_js, js_data = fetch_url(f"https://{SERVER_IP}:{PORT}/portal/qrcode.min.js", timeout=12)
check(resp_js.status == 200 and b"QRCode" in js_data and len(js_data) > 10000,
      f"Self-Contained QR Library served at /portal/qrcode.min.js (Size: {len(js_data)} bytes)")

# 2. Dynamic Subscription Endpoints (Token-based)
print("\n--- 2. Testing Token-based Dynamic Subscription Delivery ---", flush=True)
resp_sub, sub_raw = fetch_url(f"https://{SERVER_IP}:{PORT}/sub/{TOKEN}", timeout=12)
sub_json = json.loads(sub_raw.decode())

# Check Sing-box schema compliance
...
resp_b64, b64_raw = fetch_url(f"https://{SERVER_IP}:{PORT}/sub/{TOKEN}/b64", timeout=12)
decoded_links = base64.b64decode(b64_raw.decode()).decode().strip().split("\n")
...
resp_links, links_raw = fetch_url(f"https://{SERVER_IP}:{PORT}/sub/{TOKEN}/links", timeout=12)
raw_links = links_raw.decode().strip().split("\n")
...
resp_totp_path, totp_raw = fetch_url(f"https://{SERVER_IP}:{PORT}/sub/{curr_totp}", timeout=15)
totp_json = json.loads(totp_raw.decode())
...
time.sleep(0.5)
try:
    conn_rep = http.client.HTTPSConnection(SERVER_IP, PORT, timeout=15, context=ssl_ctx)
...
resp_totp_b64, b64_totp_raw = fetch_url(f"https://{SERVER_IP}:{PORT}/sub/{fresh_totp_b64}/b64", timeout=15)
totp_b64_links = base64.b64decode(b64_totp_raw.decode()).decode().strip().split("\n")
...
resp_totp_query, _ = fetch_url(f"https://{SERVER_IP}:{PORT}/sub/totp?code={fresh_totp_query}", timeout=15)
...
opener_token = urllib.request.build_opener(urllib.request.ProxyHandler({}), https_handler, urllib.request.HTTPCookieProcessor(cj_token))

# Login with Token
login_data = urllib.parse.urlencode({'auth_credential': TOKEN}).encode()
req = urllib.request.Request(f"https://{SERVER_IP}:{PORT}/portal/login", data=login_data)
resp = opener_token.open(req)
portal_html = resp.read().decode()
check(resp.geturl() == f"https://{SERVER_IP}:{PORT}/portal" and "sf_session" in [c.name for c in cj_token],
...
opener_totp = urllib.request.build_opener(urllib.request.ProxyHandler({}), https_handler, urllib.request.HTTPCookieProcessor(cj_totp))
portal_totp = wait_for_fresh_totp()
login_totp_data = urllib.parse.urlencode({'auth_credential': portal_totp}).encode()
req_totp = urllib.request.Request(f"https://{SERVER_IP}:{PORT}/portal/login", data=login_totp_data)
resp_totp = opener_totp.open(req_totp, timeout=10)
check(resp_totp.geturl() == f"https://{SERVER_IP}:{PORT}/portal" and "sf_session" in [c.name for c in cj_totp],
...
opener_bad = urllib.request.build_opener(urllib.request.ProxyHandler({}), https_handler, urllib.request.HTTPCookieProcessor(cj_bad))
bad_data = urllib.parse.urlencode({'auth_credential': 'wrong_password_or_code'}).encode()
req_bad = urllib.request.Request(f"https://{SERVER_IP}:{PORT}/portal/login", data=bad_data)
try:
    resp_bad = opener_bad.open(req_bad, timeout=10)
...
for probe_url in [
    f"https://{SERVER_IP}:{PORT}/sub/attacker_token_12345",
    f"https://{SERVER_IP}:{PORT}/wp-admin/setup-config.php"
]:
...
if current_step <= last_step:
    wait = (last_step + 1) * 30 - int(time.time()) + 5
    if wait > 0:
...
askpass_path = os.path.join(BASE_DIR, "askpass.bat")
